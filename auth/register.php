...
if(isset($_SESSION['id_user'])){
  if($_SESSION['id_level']==1||$_SESSION['id_level']==2) header('Location: ../admin/dashboard.php');
...
if(isset($_POST['register'])){
  ...
  if(empty($nama)||empty($username)||empty($password)){$error='Semua kolom wajib diisi.';}
  ...
  elseif($password!==$confirm){$error='Konfirmasi password tidak cocok.';}
    ...
    if(mysqli_num_rows($cek)>0){$error='Username sudah digunakan.';}
      ...
      if($ins) $success='Pendaftaran berhasil! Silakan login.';
      else $error='Terjadi kesalahan: '.mysqli_error($conn);
    }
  }
}
?>
...
.wrap{position:relative;z-index:10;width:100%;max-width:560px;animation:slideUp .6s cubic-bezier(.16,1,.3,1) both;}
@keyframes slideUp{from{opacity:0;transform:translateY(24px);}to{opacity:1;transform:none;}}

.brand{display:flex;align-items:center;gap:12px;justify-content:center;margin-bottom:2rem;}
.brand-icon{
...
.brand-text .name{font-size:1.4rem;font-weight:800;color:#fff;letter-spacing:-.025em;}
.brand-text .name span{color:#93c5fd;}
.brand-text .tag{font-size:.6rem;color:rgba(255,255,255,0.4);font-weight:500;letter-spacing:.12em;text-transform:uppercase;}

.card{
  background:rgba(255,255,255,0.97);border-radius:28px;padding:36px 34px 30px;
  box-shadow:0 32px 80px -12px rgba(10,26,48,0.5),0 0 0 1px rgba(255,255,255,0.1);
}
.card-title{font-size:1.55rem;font-weight:800;color:var(--p900);letter-spacing:-.03em;margin-bottom:4px;}
.card-sub{font-size:.83rem;color:var(--s500);margin-bottom:24px;}
...
.alert{display:flex;align-items:center;gap:10px;border-radius:14px;padding:11px 14px;margin-bottom:18px;}
...
.alert-ok i{color:var(--green);font-size:.82rem;flex-shrink:0;}
.alert p{font-size:.82rem;font-weight:600;}
.alert-err p{color:#b91c1c;}
.alert-ok p{color:#15803d;}
.alert-ok a{color:var(--p700);font-weight:700;text-decoration:none;}
...
.sec-lbl{font-size:.68rem;font-weight:800;color:var(--p600);text-transform:uppercase;letter-spacing:.08em;}
.sec::after{content:'';flex:1;height:1.5px;background:var(--s200);}

.grid2{display:grid;grid-template-columns:1fr 1fr;gap:14px;}
@media(max-width:480px){.grid2{grid-template-columns:1fr;}}

.field{margin-bottom:14px;}
.field-label{display:block;font-size:.68rem;font-weight:700;color:var(--s700);text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px;}
...
.finput{
  ...
  color:var(--s900);background:var(--s50);outline:none;transition:all .2s;
}
...
.feye{
  ...
  color:var(--s400);font-size:.8rem;padding:4px 6px;transition:color .2s;border-radius:6px;
}
...
.sb{flex:1;height:3px;border-radius:2px;background:var(--s200);transition:background .3s;}
.str-hint{font-size:.67rem;color:var(--s400);margin-top:3px;font-weight:500;}

.cb-terms{display:flex;align-items:flex-start;gap:9px;margin:20px 0 22px;}
.cb-terms input{width:16px;height:16px;accent-color:var(--p600);flex-shrink:0;margin-top:2px;cursor:pointer;}
.cb-terms label{font-size:.8rem;color:var(--s500);line-height:1.5;cursor:pointer;}
.cb-terms a{color:var(--p600);font-weight:600;text-decoration:none;}
...
.btn-primary{background:linear-gradient(135deg,var(--p700),var(--p500));color:#fff;box-shadow:0 6px 20px -4px rgba(30,58,102,0.45);}
.btn-primary::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,var(--p600),var(--p400));opacity:0;transition:opacity .25s;}
...
.flink{text-align:center;font-size:.83rem;color:var(--s400);margin-top:14px;}
.flink a{color:var(--p600);font-weight:700;text-decoration:none;border-bottom:1.5px solid var(--p200);}
.flink a:hover{color:var(--p700);}
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <?php if($error):?><div class="alert alert-err"><i class="fas fa-exclamation-circle"></i><p><?php echo htmlspecialchars($error);?></p></div><?php endif;?>
    <?php if($success):?><div class="alert alert-ok"><i class="fas fa-check-circle"></i><p><?php echo htmlspecialchars($success);?> <a href="login.php">Login &rarr;</a></p></div><?php endif;?>

    <form method="POST" id="rf">
      <div class="sec"><div class="sec-num">1</div><span class="sec-lbl">Identitas Diri</span></div>
      <div class="grid2">
        <div class="field">
          <label class="field-label">Nama Lengkap <sup>*</sup></label>
          <div class="fw"><i class="fi fas fa-id-card"></i>
            <input type="text" name="nama_lengkap" class="finput" placeholder="Nama lengkap" required value="<?php echo htmlspecialchars($_POST['nama_lengkap']??'');?>">
          </div>
        </div>
        <div class="field">
          <label class="field-label">Nama Pengguna <sup>*</sup></label>
          <div class="fw"><i class="fi fas fa-at"></i>
            <input type="text" name="username" class="finput" placeholder="Nama pengguna unik" required value="<?php echo htmlspecialchars($_POST['username']??'');?>">
          </div>
        </div>
      </div>

      <div class="sec"><div class="sec-num">2</div><span class="sec-lbl">Kontak</span></div>
      <div class="field">
        <label class="field-label">No. Telepon</label>
        <div class="fw"><i class="fi fas fa-phone"></i>
          <input type="tel" name="telp" class="finput" placeholder="08xx-xxxx-xxxx" value="<?php echo htmlspecialchars($_POST['telp']??'');?>">
        </div>
      </div>

      <div class="sec"><div class="sec-num">3</div><span class="sec-lbl">Keamanan</span></div>
      <div class="grid2">
        <div class="field">
          <label class="field-label">Kata sandi <sup>*</sup></label>
          <div class="fw"><i class="fi fas fa-lock"></i>
            <input type="password" name="password" id="p1" class="finput" placeholder="Min. 6 karakter" required oninput="chkStr(this.value)">
            <button type="button" class="feye" onclick="tog('p1','e1')"><i class="fas fa-eye" id="e1"></i></button>
          </div>
          <div class="str-wrap">
            <div class="str-bars"><div class="sb" id="s1"></div><div class="sb" id="s2"></div><div class="sb" id="s3"></div><div class="sb" id="s4"></div></div>
        ...
        <div class="field">
          ...
          <div class="fw"><i class="fi fas fa-lock-open"></i>
            <input type="password" name="confirm_password" id="p2" class="finput" placeholder="Ulangi kata sandi" required>
      ...
      <div class="cb-terms">
        ...
        <label for="tc">Saya menyetujui <a href="#">Syarat &amp; Ketentuan</a> dan <a href="#">Kebijakan Privasi</a> LelangOnline</label>
      </div>
